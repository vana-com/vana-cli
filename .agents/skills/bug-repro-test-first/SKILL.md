...
1. Reproduce or inspect the failure enough to choose the narrowest useful test. Tests live under `test/` mirroring `src/` (`test/cli`, `test/core`, `test/runtime`, `test/server`, `test/connectors`) and use Vitest, with `vi.stubGlobal("fetch", mockFetch)` for HTTP and a temporary `VANA_HOME` for state.
...
3. Run that test and confirm it fails for the expected reason:
   ...
   ```bash
   pnpm test test/<area>/<file>.test.ts
   ```

   No build is needed for unit tests. Tests that exercise the built CLI binary or the install lanes do need `pnpm build` first.
