...
2. Run the nearest focused checks with the repo's own scripts. Completion means
   ...
   - This repo is pnpm-only; never suggest `npm` commands here. `pnpm validate` is `pnpm lint` (`tsc --noEmit`) + `pnpm lint:eslint` + `pnpm format:check` + `pnpm test`, which is exactly what the CI `test` job runs.
   - Focused tests: `pnpm test test/<area>/<file>.test.ts`. E2E lives behind `pnpm test:e2e`.
   - CLI surface changes also carry `pnpm preflight:cli` (focused tests, build, format check, regenerated demo transcripts).
   - Packaging and install lanes are proved by `pnpm build && pnpm pack:check`, `pnpm build:sea:smoke`, `pnpm sea:check`, and `pnpm test:install:unix`; run them only when the change touches build, packaging, or install.
   - `connect/` and `examples/*` are separate workspace packages that root CI path-ignores. Prove them from inside the package.
   - The commit hook runs commitlint and lint-staged, so a commit message must be a conventional commit.
...
5. Classify accepted findings before editing:
   ...
   - **Stop-and-escalate**: changes protocol, on-disk state, exported API, CLI
     flags or exit codes, packaging, release process, or another owner's
     boundary; request direction.
...
- PRs target `main`; semantic-release publishes the npm package and the release
  assets from `main`, and the `semantic-pull-request` check requires a
  conventional-commit PR title. Review never authorizes push, merge, release, or
  tag actions.
- On release, hotfix, or publish work, apply freeze discipline: fix only the
  release blocker, exact backport, crash, data-loss, install/upgrade breakage,
  or concrete security exposure. Route other findings to follow-up work.
- The published surface is the CLI binary plus `@opendatalabs/connect` entry
  points. Treat exported types, CLI flags, exit codes, and install scripts as
  public contract: a finding that changes them is stop-and-escalate, not an
  in-scope fix.
